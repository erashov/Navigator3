...
create PROCEDURE [dbo].[report_ML_83]
...
-- Отчёт по МЛ ДЭЛС (УСП), находящиеся в заморозке 
...
and ht.starttime > CONVERT(DATETIME, @start_date, 104)
and ht.starttime <= DATEADD(hh,18,CONVERT(DATETIME, @end_date, 104))
