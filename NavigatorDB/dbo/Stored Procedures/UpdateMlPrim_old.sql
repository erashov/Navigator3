...
CREATE  PROCEDURE [dbo].[UpdateMlPrim_old]
	@NumML int,
...
SET @project_manager = (select project_manager from Osnova where NumML = @NumML)

IF Exists(Select * from Osnova where NumML = @NumML)
  BEGIN
	UPDATE    Osnova
	SET  Pre_Usluga=@Pre_Usluga,
		Usluga=@Usluga,
		Nom_Kan=@Nom_Kan,
		Zakazchik=@Zakazchik,
		Manager=@Manager,
  ...
  BEGIN
	...
	SET IDENTITY_INSERT Osnova Off
  END

  IF Not Exists(Select * from UltOsnovaMLPrimExt where NumML = @NumML)
  BEGIN
	INSERT INTO UltOsnovaMLPrimExt
	VALUES(	
	@NumML,
	@CLIENT,
	@CUST,
  ...
  END ELSE
  BEGIN
    UPDATE UltOsnovaMLPrimExt
	SET 	NumML=@NumML,
		CLIENT=@CLIENT,
		CUST=@CUST,
		RESP=@RESP,
		N1_OPERATOR=@N1_OPERATOR,
		N1_OPERATOR_NAME=@N1_OPERATOR_NAME,
		N1_OWN=@N1_OWN,
		N1_ADDINFO=@N1_ADDINFO,
		N1_PORT=@N1_PORT ,
		N2_OPERATOR=@N2_OPERATOR ,
		N2_OPERATOR_NAME=@N2_OPERATOR_NAME,
		N2_OWN=@N2_OWN,
		N2_ADDINFO=@N2_ADDINFO,
		N2_PORT=@N2_PORT ,
		ETO_NUM=@ETO_NUM ,
		N1_OLD_CONTACT=@N1_OLD_CONTACT,
		N2_OLD_CONTACT=@N2_OLD_CONTACT,
		VPN=@VPN ,
		ADSL_PHONES=@ADSL_PHONES,
		PHONE_OWNER=@PHONE_OWNER,
		MGTS_PHONE=@MGTS_PHONE,
		CHAN_TYPE=@CHAN_TYPE,
