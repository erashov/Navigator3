...
CREATE	PROCEDURE [dbo].[UpdateMlPrim1]
	@NumML			int,
...
BEGIN TRAN
  ...
  DECLARE @disconnectDate	datetime

  select @project_manager = project_manager from Osnova where NumML = @NumML
  select @SpecProject     = SpecProject from Osnova where NumML = @NumML
  select @disconnectDate  = signD2 from Osnova where NumML = @NumML
