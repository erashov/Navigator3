...
CREATE PROCEDURE [dbo].[report_ML_84]
@start_date nvarchar(25),
@end_date nvarchar(25)


/*
среднее время по кол-ву выполеннных МЛ/ТЛ в ДЭЛС (УСП) на организацию новых услуг
 (учесть  условие наличия заморозок МЛ если они попадают на период выполнения работ )
*/
  ...
  select 	
   ...
   from
    List2Nav ln left join ML_History ht on ln.numml=ht.NumML and ht.Step=8
                left join Osnova os on ln.NumML = os.NumML
   where 
     ln.numML < 1000000 
     ...
     and os.Pre_Usluga like '%Новая услуга%'
