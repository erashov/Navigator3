...
CREATE FUNCTION [dbo].[Get_InfoMLForTask] (@ml int)
RETURNS TABLE
AS
RETURN 
( 
        select  theServiceId, case when  c.NumServ like '%i%' then c.NumServ else upper(c.NumServ) end NumServ, c.ServNameCmfl,
                ...
                ch_addr_a Adres_A, ch_addr_b Adres_B,o.Dop_A,o.Dop_B,
    ...
    Left join Navigator_Test.dbo.ServiceMLComment smlc on smlc.IDgroup=c.id_group
    left join Navigator_test.dbo.Osnova o on c.numML=o.NumML
    Left join Navigator_test.dbo.ML_main mlm on mlm.NumMl=c.numML
    left join Navigator_test.dbo.ML_Ends kont_a on kont_a.numml=c.NumML and lower(kont_a.okonchanie)='a'
    left join Navigator_test.dbo.ML_Ends kont_b on kont_b.numml=c.NumML and lower(kont_b.okonchanie)='b'
    left join Navigator_test.dbo.ML_Lines line_a on line_a.numml=c.NumML and lower(line_a.line_type)='a'
    left join Navigator_test.dbo.ML_Lines line_b on line_b.numml=c.NumML and lower(line_b.line_type)='b'
);
