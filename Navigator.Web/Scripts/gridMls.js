...
var columns = [
        ...
        { field: "oshugpz_start", editable: false, title: "ГФЗ", filterable: filterGfz, template: "#=oshugpz_start ? kendo.toString(oshugpz_start,'dd.MM.yyyy'):'' #<div class=\"valueDel\"></div>#=oshugpz_end ? kendo.toString(oshugpz_end,'dd.MM.yyyy'):'' # #=oshugpzIsCanceled ? oshugpzIsCanceled:'' #", width: 340 },
        ...
        { field: "Urgent", title: "Срочность", template: " <input type='checkbox' #= Sroch_USHTU ? 'checked=checked' : '' # disabled='disabled' ></input> #=Sroch_SZ?Sroch_SZ:''#", width: 130 },
        ...
        {
            field: "Sdan", editable: false, title: "Сдан", width: 100, type: "date", filterable: false,
            format: "{0:dd.MM.yyyy}"
        }, {
            field: "Sdan_TS", editable: false, title: "Сдан ТС", width: 100, type: "date", filterable: false,
            format: "{0:dd.MM.yyyy}"
        }
...
var columnUrgent = jQuery('div#gridMls span.k-filtercell[data-field="Urgent"]');
var fieldValueUrgent = jQuery('<table><tr><td style="width:30%;"><input type="text" id="Sroch_SZ" style="width:100%;" class="k-input k-textbox" role="textbox"/></td></td><td style="width:40%;"><input id="dropdownlistSroch" /></td></tr></table>');
columnUrgent.empty();
columnUrgent.append(fieldValueUrgent);
$("#dropdownlistSroch").kendoDropDownList({
    autoBind: false,
    dataTextField: "text",
    dataValueField: "value",
    dataSource: new kendo.data.DataSource({ data: [{ text: "Содержит", value: "1" },{ text: "Срочные", value: "2" },{ text: "Не срочные", value: "3" },{ text: "Не срочные выполненые", value: "4" }]}),
    index: 0,
    optionLabel: {text: "Без фильтра",value: ""},
    change: function () {
        var value = this.value();
        var ds = $("#gridMls").data("kendoGrid").dataSource;
        var new_filter = { field: "oshugpz_ReportExist", operator: "eq", value: parseInt(value) };
...
$("#dropdownlistGfz").kendoDropDownList({
    ...
    dataValueField: "value",
    dataSource: new kendo.data.DataSource({
        data: [{ text: "Выдано", value: "1" },{ text: "Не выдано", value: "2" },{ text: "Выполнено", value: "3" },{ text: "Выдано впервые", value: "4" }]}),
    index: 0,
    optionLabel: {text: "Без фильтра",value: ""},
    change: function () {
        var value = this.value();
        ...
        if (value) {
            ...
            { 
                ...
                filterD = { field: "oshugpz_start", operator: "gte", value: kendo.toString(from.value(), "dd.MM.yyyy") };
                filterto = { field: "oshugpz_start", operator: "lte", value: kendo.toString(to.value(), "dd.MM.yyyy") };
                if (ds.filter() != null) {
                    curr_filters = ds.filter().filters;
                    curr_filters = removeFilter(curr_filters, 'oshugpz_ReportExist');
                    curr_filters.push(new_filter);
                    curr_filters.push(filterD);
...
function buttonClickClearFilters() {
